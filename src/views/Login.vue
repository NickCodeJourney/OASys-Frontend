<template>
  <div class="login">
    <el-form :model="forms" :rules="rules" ref="formRef" class="login-form">
      <h3 class="title">
        OA管理系统
      </h3>
      ...
      <el-form-item>
        <el-button type="primary" @click="onSubmit" :loading="load" round>登录</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
  ...
  methods: {
    onSubmit() {
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.load = true;
          login(qs.stringify(this.forms)).then(response => {
            this.load = false;
